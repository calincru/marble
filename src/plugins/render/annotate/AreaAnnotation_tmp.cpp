...
{
    foreach ( const QRegion &region, m_boundariesList ) {
        if ( region.contains( point ) ) {
            return true;
        }
    }

    foreach ( const PolygonNode &node, m_outerNodesList ) {
    ...
    foreach ( const QList<PolygonNode> &nodesList, m_innerNodesList ) {
        foreach ( const PolygonNode &node, nodesList ) {
            if ( node.containsPoint( point ) ) {
                return true;
            }
        }
